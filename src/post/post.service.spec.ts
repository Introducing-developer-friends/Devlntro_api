...
describe('PostService', () => {
  ...
  describe('createPost', () => {
    ...
    it('should handle database error during post creation', async () => {
      mockPostRepository.createQueryBuilder().execute.mockRejectedValueOnce(new Error('DB Error'));

      await expect(service.createPost(1, {
        content: 'Test',
        imageUrl: ''
      }))
        .rejects
    ...
    it('should handle invalid user_id during post creation', async () => {
      mockPostRepository.createQueryBuilder().execute
        .mockRejectedValueOnce({ code: 'ER_NO_REFERENCED_ROW_2' });

      await expect(service.createPost(999, {
        content: 'Test',
        imageUrl: 'test-url'
  ...
  describe('updatePost', () => {
    ...
    it('should update a post successfully', async () => {
      const updateData: PostUpdateData = { content: 'Updated', imageUrl: 'new-url' };
    ...
    it('should handle S3 deletion failure gracefully', async () => {
      ...
      await expect(service.updatePost(1, 1, { imageUrl: 'new-url' }))
        .resolves
        .not.toThrow();
    });
    ...
    it('should update only content when imageUrl is not provided', async () => {
      ...
      expect(mockPostRepository.createQueryBuilder().select).not.toHaveBeenCalled();
    ...
    it('should throw NotFoundException when post not found during image update', async () => {
      mockPostRepository.createQueryBuilder().getOne.mockResolvedValueOnce(null);
    ...
    it('should throw NotFoundException when trying to update with wrong userId', async () => {
      ...
      // execute는 호출되지 않아야 함
      mockPostRepository.createQueryBuilder().execute.mockResolvedValueOnce(undefined);
  
      await expect(service.updatePost(2, 1, { imageUrl: 'new-url' }))
        .rejects
        .toThrow(NotFoundException);
  ...
  describe('deletePost', () => {
    ...
    it('should handle invalid image URL during deletion', async () => {
      ...
      await expect(service.deletePost(1, 1))
        .resolves
        .not.toThrow();
    });
  ...
  describe('likePost', () => {
    it('should handle case when affected rows is 0 during unlike', async () => {
      mockPostLikeRepository.createQueryBuilder().execute.mockRejectedValueOnce({
        code: 'ER_DUP_ENTRY'
      });
    ...
    it('should handle database error during like operation', async () => {
      ...
      await expect(service.likePost(1, 1))
        .rejects
        .toThrow(InternalServerErrorException);
    });

    it('should return 0 like count when post_like_count is null', async () => {
      mockPostLikeRepository.createQueryBuilder().execute.mockResolvedValueOnce({});
      mockPostRepository.createQueryBuilder().execute.mockResolvedValueOnce({});
      mockPostRepository.findOne.mockResolvedValueOnce({
    ...
    it('should handle consecutive like/unlike operations by same user', async () => {
      ...
      mockPostLikeRepository.createQueryBuilder().execute
    ...
    it('should handle concurrent like operations correctly', async () => {
      ...
      // 두 번째 시도: 좋아요 삭제
      mockPostLikeRepository.delete
        .mockResolvedValueOnce({ affected: 1 });
      
      // 최종 좋아요 수 조회
      mockPostRepository.findOne
        .mockResolvedValueOnce({ post_like_count: 0 });
    ...
    it('should maintain proper like count on null values', async () => {
      ...
      mockPostRepository.createQueryBuilder().execute
        .mockResolvedValueOnce({});
      mockPostRepository.findOne
        .mockResolvedValueOnce({ post_like_count: null });
  ...
  describe('edge cases', () => {
    it('should handle empty update data', async () => {
      ...
      await expect(service.updatePost(1, 1, {}))
        .resolves
        .not.toThrow();
    });
