...
export class NotificationsService {
  ...
  async getNotifications(userId: number): Promise<NotificationInfo[]> {
    try {
      ...
      const notifications = await this.notificationRepository
        ...
        .where('notification.receiver.user_id = :userId', { userId })
  ...
  async createNotification(data: NotificationCreateData): Promise<{ notificationId: number }> {
    ...
    const [receiver, sender] = await Promise.all([
      this.userRepository.findOne({ where: { user_id: data.receiverId } }),
      this.userRepository.findOne({ where: { user_id: data.senderId } })
    ]);
    ...
    if (!sender) {
      throw new NotFoundException('알림을 보내는 사용자를 찾을 수 없습니다.');
    }
