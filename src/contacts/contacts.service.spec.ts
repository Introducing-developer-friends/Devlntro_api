...
describe('ContactsService', () => {
 ...
 beforeEach(async () => {
   ...
   const module: TestingModule = await Test.createTestingModule({
     providers: [
       ContactsService,
       { provide: getRepositoryToken(BusinessContact), useValue: mockContactRepository },
 ...
 describe('getContactList', () => {
   ...
   it('should filter out deleted users', async () => {
    ...
    expect(queryBuilder.andWhere).toHaveBeenCalledWith('userAccount.deletedAt IS NULL');
    expect(queryBuilder.andWhere).toHaveBeenCalledWith('contact_user.deletedAt IS NULL');
  });
 ...
 describe('getContactDetail', () => {
   ...
   it('should throw NotFoundException', async () => {
     ...
     await expect(service.getContactDetail(1, 999))
       .rejects
   ...
   it('should handle deleted user access attempt', async () => {
    ...
    await expect(service.getContactDetail(1, 2))
      .rejects
      .toThrow(NotFoundException);
  });
 });
 ...
 describe('addContactRequest', () => {
     ...
     mockFriendRequestRepository.save.mockResolvedValue({ request_id: 1 } as FriendRequest);
   ...
   it('should reject self-request', async () => {
     ...
     await expect(service.addContactRequest(1, 'user1'))
       .rejects
   ...
   it('should throw ConflictException for existing request', async () => {
    ...
    await expect(service.addContactRequest(1, 'user2'))
      .rejects
  ...
  it('should throw ConflictException for existing contact', async () => {
    mockUserRepository.findOne
      ...
      .mockResolvedValueOnce(mockUsers[1]);
    mockFriendRequestRepository.findOne.mockResolvedValue(null);
    mockContactRepository.findOne.mockResolvedValue({ contact_id: 1 } as BusinessContact);
  ...
  it('should handle request to deleted user', async () => {
    ...
    await expect(service.addContactRequest(1, 'deleted-user'))
      .rejects
      .toThrow(BadRequestException);
  });
  ...
  it('should handle request with non-existent login id', async () => {
    ...
    await expect(service.addContactRequest(1, 'non-existent'))
      .rejects
      .toThrow(BadRequestException);
  });
 ...
 describe('acceptContactRequest', () => {
  ...
  it('should handle non-pending request', async () => {
    ...
    const mockRequest: Partial<FriendRequest> = {
      request_id: 1,
      sender: mockUsers[0] as UserAccount,
      receiver: mockUsers[1] as UserAccount,
    ...
    // transaction mock 수정
    (dataSource.transaction as jest.Mock).mockRejectedValue(new NotFoundException());

    await expect(service.acceptContactRequest(2, 1))
      .rejects
      .toThrow(NotFoundException);
  });
 ...
 describe('getReceivedRequests', () => {
   ...
   it('should only return pending requests', async () => {
    ...
    expect(queryBuilder.andWhere).toHaveBeenCalledWith('request.status = :status', { status: 'pending' });
  });
 });
 ...
 describe('deleteContact', () => {
  ...
  // 연락처를 성공적으로 삭제하는 경우를 테스트
   ...
   it('should throw NotFoundException', async () => {
     ...
     await expect(service.deleteContact(1, 999))
       .rejects
       .toThrow(NotFoundException);
   });
  ...
  it('should handle soft delete error gracefully', async () => {
    ...
    mockContactRepository.findOne.mockResolvedValue(mockContact);
    mockContactRepository.softRemove.mockRejectedValue(new Error('Delete failed'));
